...
class ToApp:
    ...
    def get_shelf(self):
        ...
        for i in range(len(shelf)):
            ...
            if not book.available:
                self.logger.debug(consts.lang["utils"]["ToApp"]["not_avai"])
                continue
            if book.idx == 0:
                self.logger.debug(consts.lang["utils"]["ToApp"]["no_chap"])
                # continue
    ...
    def choose_single(self):
        chaps = input(consts.lang["utils"]["ToApp"]["retry_pro"]).strip().split(" ")
    ...
    def _testIP(self, ip: str):
        if not match(r"(\d{1,3}\.){3}\d{1,3}", ip):
            self.logger.debug(consts.lang["utils"]["ToApp"]["re_fail"])
            return False
        try:
            res = get(consts.GET_SHELF.format(ip), timeout=config.TIMEOUT)
            self.logger.debug(consts.lang["utils"]["ToApp"]["http_dbg"] % res.status_code)
            if res.status_code != 200:
                raise ServerError(res.status_code)  # type: ignore
            return True
        except RequestException:
            self.logger.debug(consts.lang["utils"]["ToApp"]["ser_fail"])
            return False
        except Exception as e:
    ...
    def getIP(self):
        ...
        while True:
            ...
            if self._testIP(ip):
                ...
                self.logger.debug("_testIP() returned False")
                print(consts.lang["utils"]["ToApp"]["app_fail"])

    def saveIP(self):
        if isdir("ip.conf"):
            print(consts.lang["utils"]["ToApp"]["dir_fail1"])
            print(consts.lang["utils"]["ToApp"]["dir_fail2"])
            e = FileExistsError(consts.lang["utils"]["ToApp"]["dir_fail3"])
...
class ToServer:
    def __init__(self, optDir):
        ...
        self.total_time = 0
        self.createdir()
        self.logger.debug(consts.lang["utils"]["ToSer"]["init"])
    ...
    def _deal(self, ret: SpeechSynthesisResult, j, retry: set[Chapter], chapters, bar):
        ...
        bar.text = consts.lang["utils"]["ToSer"]["retry_pro"] % (
