...
async def transferMsTTSData(SSML_text, outputPath):
    ...
    logger.debug("[{}]req_id: {}".format(outputPath.split("_")[0],req_id))
    ...
    async with websockets.connect(endpoint2) as websocket:  # type: ignore
        ...
        audio_stream = b''
        logger.debug("[{}]Begin to download.".format(outputPath.split("_")[0]))
        while(True):
