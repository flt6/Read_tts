...
async def transferMsTTSData(SSML_text, outputPath):
    ...
    async with websockets.connect(endpoint2) as websocket:
        ...
        payload_3 = SSML_text
        # print(req_id,getXTime(),payload_3)
        message_3 = 'Path: ssml\r\nX-RequestId: ' + req_id + '\r\nX-Timestamp: ' + \
...
if __name__ == "__main__":
    ...
    getLogger(bool(config[0]),bool(config[1]))
    opt_path=stdin.readline().strip()
    SSML_text=stdin.read()
    asyncio.get_event_loop().run_until_complete(mainSeq(SSML_text, opt_path))